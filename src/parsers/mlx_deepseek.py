...
def _profile_model(m: nn.Module, config: Any, B:int = 1, L:int = 4096, a_dtype=mx.float16, w_dtype=mx.float32):
        ...
        if isinstance(l, DeepseekV2DecoderLayer):
            ...
            for k in l.leaf_modules():
                ...
                # MLA Block 
                    ...
                    for key, leaf in l[k].named_modules()[::-1]: # Hack to reverse the symbol order
                        if key == "": continue
                        ...
                        if key == "q_a_proj":
                            lm.weight_bytes = config.q_lora_rank*config.hidden_size*w_dtype.size
                            lm.input_bytes = B*L*config.hidden_size*a_dtype.size
                            lm.output_bytes = B*L*config.q_lora_rank*a_dtype.size
                            lm.flops = 2*B*L*config.hidden_size*config.q_lora_rank
                        ...
                        # (B, L, q_lora_rank) -> (B, L, num_heads*q_head_dim)
                            ...
                            lm.weight_bytes = config.num_attention_heads*q_head_dim*config.q_lora_rank*w_dtype.size 
                            lm.input_bytes = B*L*config.q_lora_rank*a_dtype.size
                            lm.output_bytes = B*L*config.num_attention_heads*q_head_dim*a_dtype.size
                            lm.flops = 2*B*L*(config.num_attention_heads*q_head_dim)*config.q_lora_rank
                        ...
                        # (B, L, D) -> (B, L, kv_lora_rank+qk_rope_head_dim)
                            ...
                            lm.weight_bytes = out_features*config.hidden_size*w_dtype.size
                            lm.input_bytes = B*L*config.hidden_size*a_dtype.size
                        ...
                        # (B, L, num_heads*(q_head_dim - v_head_dim + qk_rope_head_dim))
                            ...
                            q_head_dim = config.qk_nope_head_dim*config.qk_rope_head_dim
                            out_features = config.num_attention_heads * (config.qk_nope_head_dim + config.v_head_dim) 
                            lm.weight_bytes = out_features*config.kv_lora_rank*w_dtype.size
                        ...
                        # (B, L, num_heads*v_head_dim) -> (B, L, D)
                            ...
                            lm.output_bytes = lm.input_bytes = B*L*config.kv_lora_rank*a_dtype.size 
                            submodules.append(lm)
                            continue
                    ...
                    if (config.n_routed_experts is not None and 
                        ...
                        for key, leaf in l[k].named_modules()[::-1]:
                            ...
                            if key == "gate":
                                ...
                                lm.layer = leaf
                                lm.name = key 
                                lm.weight_bytes = config.hidden_size*config.n_routed_experts*a_dtype.size
                                lm.input_bytes = B*L*config.hidden_size*a_dtype.size
                                lm.output_bytes = B*L*config.n_routed_experts*a_dtype.size
                                lm.flops = 2*B*L*config.hidden_size*config.n_routed_experts
                                submodules.append(lm)
                            ...
                            # SwitchGLU (B, L, D) -> (B, L, D)
                                ...
                                for key2, proj in leaf.named_modules()[::-1]:
                                    ...
                                    if key2 == "gate_proj":
                                        ...
                                        lm.input_bytes = B*L*config.num_experts_per_tok*config.hidden_size*a_dtype.size
                                        lm.output_bytes = B*L*config.num_experts_per_tok*config.moe_intermediate_size*a_dtype.size
                                        lm.flops = 2*(B*L)*config.num_experts_per_tok*DS
                                    ...
                                    # ((B*L)*num_experts_per_tok, moe_intermediate_size) 
                                        ...
                                        lm.flops = (B*L)*config.num_experts_per_tok*config.moe_intermediate_size

                                    # ((B*L)*num_experts_per_tok, moe_intermediate_size) -> 
                                    # ((B*L)*num_experts_per_tok, D) 
                                    elif key2 == "down_proj":
                                        lm.weight_bytes = DS*a_dtype.size
                                        lm.input_bytes = B*L*config.num_experts_per_tok*config.moe_intermediate_size*a_dtype.size
                                        lm.output_bytes = B*L*config.num_experts_per_tok*config.hidden_size*a_dtype.size
                                        lm.flops = 2*(B*L)*config.num_experts_per_tok*DS
                                ...
                                for key2, proj in leaf.named_modules()[::-1]:
                                    ...
                                    if key2 == "gate_proj" or key2 == "up_proj":
                                        ...
                                        lm.weight_bytes = S_sh*config.hidden_size*a_dtype.size
                                        lm.input_bytes = B*L*config.hidden_size*a_dtype.size
                                        lm.output_bytes = B*L*config.n_shared_experts*config.moe_intermediate_size*a_dtype.size
                                        lm.flops = 2*(B*L)*config.hidden_size*S_sh

                                    # ((B*L), n_shared_experts*moe_intermediate_size) -> ((B*L), D)
                                    elif key2 == "down_proj":
                                        S_sh = config.n_shared_experts*config.moe_intermediate_size
                                        lm.weight_bytes = S_sh*config.hidden_size*a_dtype.size
                                        lm.input_bytes = B*L*config.n_shared_experts*config.moe_intermediate_size*a_dtype.size
                                        lm.output_bytes = B*L*config.hidden_size*a_dtype.size
                                        lm.flops = 2*(B*L)*config.hidden_size*S_sh
                    ...
                    # Compute MLP data 
                        ...
                        for key, leaf in l[k].named_modules()[::-1]:
                            ...
                            if key == "gate_proj" or key == "up_proj":
                                lm.weight_bytes = config.intermediate_size*config.hidden_size*a_dtype.size
                                lm.input_bytes = B*L*config.hidden_size*a_dtype.size 
                                lm.output_bytes = B*L*config.intermediate_size*a_dtype.size
                                lm.flops = 2*(B*L)*config.hidden_size*config.intermediate_size

                            # ((B*L), intermediate_size) -> ((B*L), D)
                            elif key == "down_proj":
                                lm.weight_dtype = config.intermediate_size*config.hidden_size*a_dtype.size
                                lm.input_bytes = B*L*config.intermediate_size*a_dtype.size
                                lm.output_bytes = B*L*config.hidden_size*a_dtype.size 
                                lm.flops = 2*(B*L)*config.hidden_size*config.intermediate_size
            ...
            if decoder_idx == 1:
                ...
                emlm = LayerMeta()
                emlm.name = "embedding"
                emlm.weight_bytes = config.vocab_size * config.hidden_size * w_dtype.size 
                emlm.output_bytes = B*L*config.hidden_size*a_dtype.size
                lm_decoder.submodules.insert(0, emlm)

            # decoder-scoped KV cache bytes 
            lm_decoder.kv_cache_r = B*(config.num_key_value_heads*L*(config.qk_nope_head_dim+config.qk_rope_head_dim) + \
                                    config.num_attention_heads*L*config.v_head_dim) * a_dtype.size
            lm_decoder.kv_cache_w = B*(config.num_key_value_heads * (config.qk_nope_head_dim + config.qk_rope_head_dim) + \
