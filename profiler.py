...
def _mlx_gemm_benchmark(device, N, M, K, warmup:int = 3, iters:int = 10, dtype=mx.float32) -> float:
...
def run_cpu_benchmarks(device_info):
    M=N=K = 2<<8
    device_info.cpu.benchmarks.flops_f64  = _mlx_gemm_benchmark(mx.cpu, N,M,K, 3, 10, mx.float64)
    device_info.cpu.benchmarks.flops_f32  = _mlx_gemm_benchmark(mx.cpu, N,M,K, 3, 10, mx.float32)
    device_info.cpu.benchmarks.flops_fp16 = _mlx_gemm_benchmark(mx.cpu, N,M,K, 3, 10, mx.float16)
    device_info.cpu.benchmarks.flops_bf16 = _mlx_gemm_benchmark(mx.cpu, N,M,K, 3, 10, mx.bfloat16)
...
def run_gpu_benchmarks(device_info):
    ...
    device_info.gpu.benchmarks.flops_f32  = _mlx_gemm_benchmark(mx.gpu, N,M,K, 3, 10, mx.float32)
    device_info.gpu.benchmarks.flops_fp16 = _mlx_gemm_benchmark(mx.gpu, N,M,K, 3, 10, mx.float16)
    device_info.gpu.benchmarks.flops_bf16 = _mlx_gemm_benchmark(mx.gpu, N,M,K, 3, 10, mx.bfloat16)
...
def bench_cpu_to_gpu_transfers(di):
    if _has_cupy:
        ...
        def cpu_to_gpu():
            with sec_cpu2gpu:
                cp.cuda.runtime.memcpyAsync(
                    d.data.ptr, h_in_ptr, bytes_total,
                    cp.cuda.runtime.memcpyHostToDevice, sec_cpu2gpu.ptr)

        def gpu_to_cpu():
            with sec_gpu2cpu:
                cp.cuda.runtime.memcpyAsync(
                    h_out_ptr, d.data.ptr, bytes_total,
                    cp.cuda.runtime.memcpyDeviceToHost, sec_gpu2cpu.ptr)
        ...
        di.gpu.memory.read_write_bw = (2*bytes_total/bench(read_write, sec_rw)) * 1e-9
...
def get_sysmem_info(device_info):
    ...
    device_info.memory.cpu_read_cold_bw = (bytes_A/bench(lambda: mx.max(A), 0, 1)) / 1e9
    ...
    device_info.memory.cpu_read_warm_bw = (bytes_A/bench(lambda: mx.abs(A), 5, 10))/1e9
